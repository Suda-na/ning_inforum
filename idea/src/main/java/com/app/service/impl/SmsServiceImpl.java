...
public class SmsServiceImpl implements SmsService {
    ...
    public String[] sendVerificationCodeWithDypns(String phone) {
        try {
            ...
            // 如果签名名称看起来像是被错误编码（ISO-8859-1 当作 UTF-8 读取），尝试修复
            // 检查是否包含乱码字符（通常是 ISO-8859-1 被当作 UTF-8 读取的结果）
            boolean needsFix = false;
            try {
                // 尝试将字符串按 ISO-8859-1 解码，然后按 UTF-8 编码
                String testFix = new String(signName.getBytes(StandardCharsets.ISO_8859_1), StandardCharsets.UTF_8);
                // 如果修复后的字符串包含中文字符且长度合理，使用修复后的值
            ...
            // 模板参数：根据阿里云 dypns API 文档
            // 格式：{"code":"##code##","min":"5"} 或 {"code":"123456","min":"5"}
            // 注意：code 参数名需要替换成实际模板中的参数名称
            // 使用 ##code## 时，验证码由 API 动态生成，阿里云可以完成校验
            // min 参数必须是字符串格式
            int expireMinutes = 5;
            
            // 获取模板参数名称（从配置读取，默认为 code）
            ...
            // 构建模板参数 JSON
            // 根据文档，参数名需要与模板中的参数名称一致
            // 例如：如果模板参数名是 verifyCode，则使用 {"verifyCode":"##code##","min":"5"}
