...
public class WErrandRepository {
    ...
    private static WErrandOrder parseOrderFromJson(JSONObject json) throws Exception {
        ...
        if (json.has("publisherId")) {
            publisherId = json.optInt("publisherId", 0);
            if (publisherId == 0) publisherId = null;
        } else if (json.has("userId")) {
            publisherId = json.optInt("userId", 0);
            if (publisherId == 0) publisherId = null;
        }
        Log.d(TAG, "订单publisherId: " + publisherId);
        
        Integer postId = null;
        if (json.has("postId")) {
            postId = json.optInt("postId", 0);
            if (postId == 0) postId = null;
        }
        Log.d(TAG, "订单postId: " + postId);
        ...
        WErrandOrder order = new WErrandOrder(id, tag, title, desc, from, to, price, status, images, publisherId, postId);
    ...
    public static WErrandOrder getOrder(String id) {
        try {
            ...
            if (response.success && response.getCode() == 200) {
                ...
                if (dataObj instanceof JSONObject) {
                    JSONObject data = (JSONObject) dataObj;
                    WErrandOrder order = parseOrderFromJson(data);
                    Log.d(TAG, "获取订单详情成功: id=" + (order != null ? order.getId() : "null"));
                    return order;
                }
            } else {
