...
public class UserController {
    ...
    // GET 请求示例
    @GetMapping("/info/{id}")
    public CResult<User> getUserInfo(@PathVariable Integer id) {
        // 业务逻辑...
        return CResult.success(user);
    }

    // POST 请求示例
    @PostMapping("/login")
    public CResult<LoginResponse> login(@RequestBody LoginRequest request) {
        // 业务逻辑...
        return CResult.success(loginResponse);
    }

    // 错误响应示例
    @GetMapping("/error")
    public CResult<?> testError() {
        return CResult.error("这是一个错误示例");
    }
}
```
