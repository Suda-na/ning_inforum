...
public class CClientController {
    ...
    /**
     * 检查是否已关注
     * @param userId 当前用户ID（followerId）
     * @param targetUserId 目标用户ID（followingId）
     * @return 是否已关注
     */
    ...
    public CResult<List<Map<String, Object>>> getMyPosts(@RequestParam("userId") Integer userId) {
        try {
            ...
            // 调用WCircleService获取用户发布的动态帖子（categoryId=1）
            List<Map<String, Object>> posts = wCircleService.getUserPosts(userId, 1);
