...
/**
 * 发布失物招领Activity（S开头，调用SPostController）
 */
public class SPublishLostFoundActivity extends AppCompatActivity {
    ...
    private void handlePublish() {
        ...
        try {
            ...
            HttpUtil.post("/api/android/post/publish/lostfound", jsonData, new HttpUtil.HttpCallback() {
                @Override
                public void onSuccess(String response) {
                    runOnUiThread(() -> {
                        try {
                            ...
                            if (success) {
                                Toast.makeText(SPublishLostFoundActivity.this, message, Toast.LENGTH_SHORT).show();
                                finish();
                            } else {
                                Toast.makeText(SPublishLostFoundActivity.this, message, Toast.LENGTH_SHORT).show();
                            }
                        } catch (Exception e) {
                            Toast.makeText(SPublishLostFoundActivity.this, "解析响应失败", Toast.LENGTH_SHORT).show();
                        }
                    });
                }
                ...
                public void onError(String error) {
                    runOnUiThread(() -> {
                        Toast.makeText(SPublishLostFoundActivity.this, "发布失败: " + error, Toast.LENGTH_SHORT).show();
                    });
                }
            });
        } catch (Exception e) {
